...
class Command(BaseCommand):
    ...
    help = 'Merges all Gallery pictures from obsolete_user into canonical_user (both files and metadata).\n' \
           'Pass usernames into each parameter.\n' \
           'The default behavior is to commit the actions. Pass --readonly to see what actions this command will take, without actually doing them.\n' \
           'WARNING: Without --readonly, this command will delete the obsolete user from Django and LDAP. Use with caution.\n\n'
    ...
    def handle(self, *args, **options):
        ...
        if self.readonly:
            self.stdout.write('Output is sample only, changes NOT committed.\n')
        ...
        self.one_account_to_rule_its_doppelgangers(canonical_user, obsolete_user)


    # Takes a canonical account, and a list of other accounts, and moves the pictures to the new one.
    def one_account_to_rule_its_doppelgangers(self, canonical_user, obsolete_user):
        # For each photo owned by this user, change its owner to the canonical user.
        photos = Photo.objects.filter(owner=obsolete_user)
        artifacts = Artifact.objects.filter(owner=obsolete_user)
        self.stdout.write('Found %s photos and %s artifacts for %s.\n' % (len(photos), len(artifacts), str(obsolete_user)) )
        ...
        images = list(photos) + list(artifacts) # Convert from QuerySet to objects    
        for image in images:
            self.change_image_owner(canonical_user, image)

        # Delete the obsolete account, both the Gallery profile and the Django user.
    ...
    def change_image_owner(self, newuser, image):
        ...
        # Separate the relative directory and the filename itself:      subpath                                           oldfilename 
        subpath, old_filename = os.path.split(image.image.path) # e.g. '/var/www/bm.gallery/media/photos/andrei_rublev', 'andrei_rublev.1234.jpg'

        # Remove the author's dir from the base dir used to store images of the current type.
        images_base, old_owner_dir = os.path.split(subpath) # e.g. '/var/www/bm.gallery/media/photos', 'andrei_rublev'
        ...
        # Move the file to its new home, e.g. '/var/www/bm.gallery/media/photos/andrei/andrei.1234.jpg'
        new_file_path = images_base + '/' + newuser.username + '/' + new_filename

        # Change image metadata in Django
        image.image.upload_to =  subpath + new_filename
        image.owner = newuser

        if image.in_press_gallery:
        ...
        if not self.readonly:
            ...
            # Note: os.renames will create intermediate dirs if they don't exist.
